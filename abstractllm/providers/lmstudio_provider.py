...
# Simple adapter class for test compatibility
class LMStudioLLM:
    """
    Simple adapter around LMStudioProvider for test compatibility.
    """

    def __init__(self, model="qwen/qwen3-next-80b", api_key="lm-studio"):
        """
        Initialize an LM Studio LLM instance.

        Args:
            model: The model to use
            api_key: API key (can be anything for LM Studio)
        """
        config = {
        ...
        self.provider = LMStudioProvider(config)
    ...
    def generate(self, prompt, image=None, images=None, **kwargs):
        ...
        """
        # Add images to files if provided
        files = []
        if image:
            files.append(image)
        if images:
