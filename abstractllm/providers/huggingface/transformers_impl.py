"""
HuggingFace Transformers implementation for AbstractLLM.
...
# Configure logger
logger = logging.getLogger("abstractllm.providers.huggingface.transformers")
...
# GGUF model types mapping
GGUF_MODEL_TYPES = {
    "phi": ["phi-2", "phi2", "microsoft/phi"],
    "llama": ["llama", "llama2"],
    "mistral": ["mistral"],
    "mixtral": ["mixtral"],
}
...
def is_gguf_model(model_name: str) -> bool:
    ...
    return "gguf" in model_name.lower() or any(
        any(model_type in model_name.lower() for model_type in model_types)
        for model_types in GGUF_MODEL_TYPES.values()
    )

def get_gguf_model_type(model_name: str) -> str:
    """Get the model type for a GGUF model."""
    model_name_lower = model_name.lower()
    for model_type, patterns in GGUF_MODEL_TYPES.items():
        if any(pattern in model_name_lower for pattern in patterns):
            return model_type
    return "phi"  # Default to phi if unknown

class TransformersImplementation(AbstractLLMInterface):
    """
    ...
    def __init__(self, config: Optional[Dict[Union[str, ModelParameter], Any]] = None):
        """Initialize the Transformers implementation."""
        ...
        logger.info(f"Initialized Transformers implementation with model: {model}")
    ...
    def list_cached_models(cache_dir: Optional[str] = None) -> list:
        ...
        if cache_dir is None:
            cache_dir = TransformersImplementation.DEFAULT_CACHE_DIR
